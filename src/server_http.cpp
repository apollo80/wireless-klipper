...
static void on_update_self();
static void on_update_self_finish();
static void on_update_mcu();
static void on_update_mcu_finish();
...
static uint32_t baud_range[] = {
    9600, 14400, 19200, 28800, 38400, 38400, 57600, 76800, 115200, 230400, 250000, 256000, 460800, 576000, 921600
};
...
void init_httpServer()
{
    ...
    httpServer.on("/update_self", HTTP_POST, on_update_self_finish, on_update_self);
    httpServer.on("/update_mcu", HTTP_POST, on_update_mcu_finish, on_update_mcu);
...
#if ENABLE_DEBUG
    DEBUG_ESP_PORT.printf("(%9lu) http server started", millis());
#endif
}
...
void on_root()
{
    ...
    snprintf(convert_buffer, sizeof(convert_buffer), "%i.%i.%i-%i", firmware_version.major, firmware_version.major, firmware_version.revision, firmware_version.bugfix);
    ...
    for (int idx = 0; idx < baud_range_size; ++idx) {
        char const* opTemplate = ((moduleSettings.serialPort_baud == baud_range[idx])
    ...
    snprintf(convert_buffer, sizeof(convert_buffer), "%i.%i.%i.%i", moduleSettings.static_IPaddress[0], moduleSettings.static_IPaddress[1], moduleSettings.static_IPaddress[2], moduleSettings.static_IPaddress[3]);
    body.replace("{{static_IPaddress}}", convert_buffer);

    snprintf(convert_buffer, sizeof(convert_buffer), "%i.%i.%i.%i", moduleSettings.static_netmask[0], moduleSettings.static_netmask[1], moduleSettings.static_netmask[2], moduleSettings.static_netmask[3]);
    body.replace("{{static_netmask}}", convert_buffer);

    snprintf(convert_buffer, sizeof(convert_buffer), "%i.%i.%i.%i", moduleSettings.static_gateway[0], moduleSettings.static_gateway[1], moduleSettings.static_gateway[2], moduleSettings.static_gateway[3]);
...
#if ENABLE_DEBUG
    DEBUG_ESP_PORT.printf("(%9lu) POST form was:\n", millis());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"wifi_hostname\"        : \"%s\"\n", millis(), httpServer.arg("wifi_hostname").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"wifi_ssid\"            : \"%s\"\n", millis(), httpServer.arg("wifi_ssid").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"wifi_password\"        : \"%s\"\n", millis(), httpServer.arg("wifi_password").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"wifi_use_sta\"         : \"%s\"\n", millis(), httpServer.arg("wifi_use_sta").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"serialPort_baud\"      : \"%s\"\n", millis(), httpServer.arg("serialPort_baud").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"serialPort_rxBuffSize\": \"%s\"\n", millis(), httpServer.arg("serialPort_rxBuffSize").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"tcpServer_port\"       : \"%s\"\n", millis(), httpServer.arg("tcpServer_port").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"tcpServer_buffSize\"   : \"%s\"\n", millis(), httpServer.arg("tcpServer_buffSize").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"use_static_ip\"        : \"%s\"\n", millis(), httpServer.arg("use_static_ip").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"static_IPaddress\"     : \"%s\"\n", millis(), httpServer.arg("static_IPaddress").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"static_netmask\"       : \"%s\"\n", millis(), httpServer.arg("static_netmask").c_str());
    DEBUG_ESP_PORT.printf("(%9lu)    - \"static_gateway\"       : \"%s\"\n", millis(), httpServer.arg("static_gateway").c_str());
#endif
...
{
#if ENABLE_DEBUG
    DEBUG_ESP_PORT.printf("(%9lu) Restarting ...\n", millis());
#endif
    httpServer.send(200, "text/plain", String());
    ESP.restart();
}
...
void on_update_self()
{
    ...
    if (upload.status == UPLOAD_FILE_START) {
...
#if ENABLE_DEBUG
        DEBUG_ESP_PORT.printf("(%9lu) on_update_self(); started\n", millis());
#endif
        ...
        if (!isFinish) {
            ...
            DEBUG_ESP_PORT.printf("(%9lu) Update Success: %zu\n", millis(), upload.totalSize);
        }
#endif
    } else if (upload.status == UPLOAD_FILE_ABORTED) {
        Update.end();
#if ENABLE_DEBUG
        DEBUG_ESP_PORT.printf("(%9lu) Update was aborted\n", millis());
#endif
    }
    esp_yield();
}

void on_update_self_finish()
{
#if ENABLE_DEBUG
    DEBUG_ESP_PORT.printf("(%9lu) on_update_self_finish(); Rebooting...\n", millis());
#endif
